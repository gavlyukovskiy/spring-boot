...
import org.springframework.boot.autoconfigure.jdbc.proxy.ProxyDataSource;
import org.springframework.context.ApplicationContext;
import org.springframework.context.ApplicationContextAware;

/**
 * {@link DataSourcePoolMetadataProvider} that returns specific implementation for a real
 * {@link DataSource} extracted from a {@link ProxyDataSource}.
 *
 * @author Arthur Gavlyukovskiy
 */
class ProxyDataSourcePoolMetadataProvider implements DataSourcePoolMetadataProvider, ApplicationContextAware {
	...
	public void init() {
		...
		for (DataSourcePoolMetadataProvider delegate : delegates) {
			if (!(delegate instanceof ProxyDataSourcePoolMetadataProvider)) {
